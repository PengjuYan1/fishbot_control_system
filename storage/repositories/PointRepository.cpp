...
std::optional<PointRecord> PointRepository::find_by_native_identity(long floor_id, long map_id, long point_id) const {
    sqlite3_stmt* statement = nullptr;
    const char* sql =
        "SELECT id, name, type, x, y, theta, floor_id, map_id, point_id "
...
void PointRepository::update_point(const PointRecord& point) {
    sqlite3_stmt* statement = nullptr;
    const char* sql =
        "UPDATE points SET name = ?, type = ?, x = ?, y = ?, theta = ?, floor_id = ?, map_id = ?, point_id = ? "
        "WHERE id = ?";
    if (sqlite3_prepare_v2(db_, sql, -1, &statement, nullptr) != SQLITE_OK) {
        throw std::runtime_error(sqlite3_errmsg(db_));
    }
    ...
    sqlite3_bind_int64(statement, 7, point.map_id);
    sqlite3_bind_int64(statement, 8, point.point_id);
    sqlite3_bind_int(statement, 9, point.id);
